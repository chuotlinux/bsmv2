...
$buss = mysql_query("SELECT id, busid, seatid, pname, pmobile, start, depart, trxid, status FROM seat_details WHERE seatid='".$sid."' AND busid='".$bid."'");
    echo mysql_error();
    while ($datas = mysql_fetch_array($buss))
    {
		
		$bss = mysql_fetch_array(mysql_query("SELECT id, route, time, date, seat FROM bus_info WHERE id='".$datas[1]."'"));
		
		$tkt = mysql_fetch_array(mysql_query("SELECT id, seats FROM ticket_details WHERE busid='".$datas[1]."' AND pmobile='".$datas[4]."' AND pname='".$datas[3]."'"));
		
		$rname = mysql_fetch_array(mysql_query("SELECT routename FROM bus_route WHERE id='".$bss[1]."'"));
